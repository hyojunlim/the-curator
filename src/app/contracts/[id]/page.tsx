...
export default function ContractDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/contracts/${id}`)
      ...
      .catch(() => { setLoading(false); router.push("/history"); });
  }, [id, router]);
  ...
  function handleShare() {
    if (!contract) return;
    const text = [
      `=== CONTRACT ANALYSIS: ${contract.title} ===`,
      "",
      "SUMMARY:",
  ...
  function handleExportPDF() {
    ...
    const body = `
      ...
      <p style="color:#454652">Risk Level: ${risk_score >= 70 ? "High" : risk_score >= 40 ? "Medium" : "Low"} &bull; ${result.risks.length} clauses identified</p>
      <div class="summary"><strong>Plain-English Summary</strong><p style="margin-top:8px">${escapeHtml(result.summary)}</p></div>
      ...
      <div class="footer">Generated by The Curator &bull; ${new Date().toLocaleDateString("en-US", { dateStyle: "long" })} &bull; For informational purposes only.</div>`;
    ...
    <div className="flex min-h-screen bg-surface font-body text-on-surface">
      ...
      <div className="ml-0 lg:ml-64 flex-1 p-6 pt-16 lg:pt-6 lg:p-10 pb-24">
        ...
        <div className="flex items-center gap-2 text-xs text-on-surface-variant mb-6">
          <Link href="/history" className="hover:text-primary transition-colors">Review History</Link>
        ...
        {/* Tags */}
        <div className="mb-6">
          <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">Labels</p>
        ...
        <div className="max-w-2xl space-y-5">
          ...
          <div className="bg-surface-container-lowest rounded-xl p-5 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-1">Risk Score</p>
            ...
            <div className="text-right">
              <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold ${
                ...
                Risk Level: {risk_score >= 70 ? "High" : risk_score >= 40 ? "Medium" : "Low"}
              </span>
          ...
          {result.parties && result.parties.length >= 2 && (() => {
            ...
            return (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-sm">
                ...
                <p className="text-xs text-on-surface-variant mb-4">Select your role to see tailored advice for each clause.</p>
                <div className="grid grid-cols-3 gap-2">
                  <button onClick={() => setPerspective("none")} className={`flex flex-col items-center gap-1.5 px-3 py-3.5 rounded-xl text-xs font-bold transition-all border ${perspective === "none" ? "border-primary bg-primary/10 text-primary shadow-sm ring-2 ring-primary/20" : "border-outline-variant text-on-surface-variant hover:border-primary/40 hover:text-on-surface"}`}>
                    ...
                    <span>Neutral</span>
                    <span className={`text-[10px] font-normal ${perspective === "none" ? "text-primary/70" : "text-on-surface-variant/60"}`}>General view</span>
                  </button>
                  <button onClick={() => setPerspective("party_a")} className={`flex flex-col items-center gap-1.5 px-3 py-3.5 rounded-xl text-xs font-bold transition-all border ${perspective === "party_a" ? "border-primary bg-primary/10 text-primary shadow-sm ring-2 ring-primary/20" : "border-outline-variant text-on-surface-variant hover:border-primary/40 hover:text-on-surface"}`}>
                    ...
                    <span className={`text-[10px] font-normal ${perspective === "party_a" ? "text-primary/70" : "text-on-surface-variant/60"}`}>{partyA?.description || "갑"}</span>
                  </button>
                  <button onClick={() => setPerspective("party_b")} className={`flex flex-col items-center gap-1.5 px-3 py-3.5 rounded-xl text-xs font-bold transition-all border ${perspective === "party_b" ? "border-primary bg-primary/10 text-primary shadow-sm ring-2 ring-primary/20" : "border-outline-variant text-on-surface-variant hover:border-primary/40 hover:text-on-surface"}`}>
                    ...
                    <span className={`text-[10px] font-normal ${perspective === "party_b" ? "text-primary/70" : "text-on-surface-variant/60"}`}>{partyB?.description || "을"}</span>
                  </button>
                </div>
                {perspective !== "none" && activePartyName && (
                  <div className="mt-3 flex items-center gap-2 bg-primary/5 rounded-lg px-3 py-2">
                    ...
                    <span className="text-xs text-primary font-medium">Viewing advice as <strong>{activePartyName}</strong></span>
                  </div>
                )}
              </div>
            );
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-3 text-xs font-bold">
                    {result.risks.filter((r) => r.severity === "high").length > 0 && (
                      <span className="text-error">{result.risks.filter((r) => r.severity === "high").length} Critical</span>
                    )}
                    {result.risks.filter((r) => r.severity === "medium").length > 0 && (
                      <span className="text-secondary">{result.risks.filter((r) => r.severity === "medium").length} Caution</span>
                    )}
                    {result.risks.filter((r) => r.severity === "low").length > 0 && (
                      <span className="text-on-surface-variant">{result.risks.filter((r) => r.severity === "low").length} Advisory</span>
                    )}
                  </div>
                </div>
                <div className="space-y-3">
                  {[...result.risks]
                    ...
                    .map((risk, i) => {
                      const cfg = {
                        high: { label: "Critical", badge: "bg-error-container text-on-error-container", border: "border-l-error" },
                        medium: { label: "Caution", badge: "bg-tertiary-fixed text-tertiary", border: "border-l-secondary" },
                        low: { label: "Advisory", badge: "bg-primary-fixed text-primary", border: "border-l-secondary" },
                      }[risk.severity];
                            ...
                            <div className={`mt-3 rounded-lg p-3 ${perspective !== "none" ? "bg-primary/5 border border-primary/20" : "bg-surface-container-low"}`}>
                              <div className="flex items-center gap-1.5 mb-1.5">
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-wider ${perspective !== "none" ? "text-primary" : "text-on-surface-variant"}`}>
                                  {perspective !== "none" && activePartyName ? `${activePartyName} Advice` : "Suggestion"}
                                </span>
                              </div>
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  else showToast("Failed to delete contract");
                } catch {
